...
# システム名       ：ハイブリッド書店システム
# サブシステム名   ：マーケ
# 機能名           ：Google Cloud Strageへのローデータ送信
# ファイル名       ：GCS_SEND
# 作成者           ：Yoshiaki.Kameyama
#-------------------------------------------------------------------------
#
# 【概要】
#    Google Cloud Strageに、ローデータファイルを送信する。
#    複数ファイルかつフッター付の書店データファイルは、フッターを除去して１ファイルにまとめて送信する。
#
# 【引数】
#    第1引数 LOAD_GROUPID:ロードグループID
#
# 【返却値】
#    0 : 正常終了
#    1 : 異常終了
